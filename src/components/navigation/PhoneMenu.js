...
export default function PhoneMenu({className = ''}) {

    const closeMenu = () => {
        document.querySelector('#phone_menu').classList.replace('block', 'hidden');
        document.body.classList.remove('h-[100vh]');
        document.body.classList.remove('overflow-hidden');
    }

    return(
        <div id="phone_menu" className={cn("hidden absolute top-0 left-0 w-full min-h-full bg-black/50 z-[200]", {[className] : className !== ''})}>
            <div className="w-full h-[100vh] bg-[#141413] flex flex-col justify-center gap-y-6 relative z-[300] overflow-hidden">
                ...
                <div onClick={closeMenu}>
                    <FontAwesomeIcon icon={faClose} className="text-[#e2cb8b] text-[2rem] absolute top-[2rem] right-[2rem]" />
                </div>
